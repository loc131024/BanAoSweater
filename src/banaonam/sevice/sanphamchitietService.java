...
public class sanphamchitietService {
    ...
    public ArrayList<sanphamchitiet> getallsanphamchitiet() {
        ...
        String sql = "SELECT sanpham.masp,TENSP,TENSIZE,TENMAU,TENCHATLIEU,gia,chitietsanpham.SOLUONG,MACTSP,CHITIETSANPHAM.HINHANH FROM SANPHAM \n"
    ...
    public Integer addSPCT(sanphamchitiet SPCT) {
        Integer row = null;
        Connection cn = DB.getConnection();
        String sql = "INSERT INTO CHITIETSANPHAM (MASP, MAMS, MACL, MASIZE, HINHANH, SOLUONG, GIA, MOTA, TRANGTHAI)\n"
                + "VALUES\n"
                + "(?,?,?,?,?,?,?,?,?)";
        try {
            PreparedStatement pd = cn.prepareStatement(sql);
            ...
            pd.setNString(9, SPCT.getTRANGTHAI());
    ...
    public Integer UPDATESPCT(sanphamchitiet SPCT) {
        ...
        String sql = "update CHITIETSANPHAM\n"
                + "set MASP = ? ,MAMS = ? ,MACL = ? , MASIZE = ? ,HINHANH = ? ,SOLUONG = ? , GIA =? ,MOTA = ?, TRANGTHAI = ?\n"
                + "where MACTSP = ?";
        try {
            PreparedStatement pd = cn.prepareStatement(sql);
